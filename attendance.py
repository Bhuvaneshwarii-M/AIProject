...
lbl3 = tk.Label(window, text="Status Of Attendance", width=20, fg="black", bg="pink", height=2,
                font=('Times New Roman', 25, ' bold '))
...
message2 = tk.Label(window, text="", fg="red", bg="pink", activeforeground="green", width=60, height=4,
                    font=('times', 15, ' bold '))
...
def TakeImages():
    ...
    if not Id:
        ...
        if not cam.isOpened():
            res = "Error: Could not open camera."
            message.configure(text=res)
            return
...
def TrackImages():
    recognizer = cv2.face.LBPHFaceRecognizer_create() if cv2.__version__.startswith('3') else cv2.face.LBPHFaceRecognizer_create()#cv2.createLBPHFaceRecognizer()
